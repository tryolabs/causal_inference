...
class MultiRobustSyntheticControl(RobustSyntheticControl):
    ...
    # weightsArray:             (array) weight to scale each metric by. length of array must
    #                           be == nbrMetrics
    ...
    # arrayOfKeyToSeriesDF:
    # (array) contains a keyToSeriesDF for each metric.
    ...
    def combineMetrics(self, arrayOfKeyToSeriesDF, isForTraining):
        ...
        for i in range(0, self.nbrMetrics):
            arrayOfKeyToSeriesDF[i] = arrayOfKeyToSeriesDF[i].multiply(
                np.sqrt(self.weightsArray[i])
            )
    ...
    def fit(self, arrayOfKeyToSeriesDF):
        super(MultiRobustSyntheticControl, self).fit(
            self.combineMetrics(arrayOfKeyToSeriesDF, True)
        )

    # arrayOfKeyToSeriesDFNew:
    # (array) contains a keyToSeriesDFNew (Pandas dataframe) for each metric.
    # length of array must equal the self.nbrMetrics
    ...
    def predict(self, arrayOfKeyToSeriesDFNew):
        allPredictions = super(MultiRobustSyntheticControl, self).predict(
            self.combineMetrics(arrayOfKeyToSeriesDFNew, False)
        )
        predictionsArray = []
        singleMetricPredictionsLength = int((1.0 / float(self.nbrMetrics)) * len(allPredictions))
        for i in range(0, self.nbrMetrics):
            predictions = allPredictions[
                i * singleMetricPredictionsLength: (i + 1) * singleMetricPredictionsLength
            ]
